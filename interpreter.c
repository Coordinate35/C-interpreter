 ...
 */
enum {
    LEA, IMM, JMP, CALL, JZ, JNZ, ENT, ADJ, LEV, LI, LC, SI, SC, PUSH, OR, XOR, AND, EQ, NE, LT, GT, LE, GE,
    SHL, SHR, ADD, SUB, MUL, DIV, MOD, OPEN, READ, CLOS, PRTF, MALC, MSET, MCMP, EXIT
}; 

/**
 * token and classes (operators last and in preedence order)
 */
enum {
    Num = 128, Fun, Sys, Glo, Loc, Id, Char, Else, Enum, If, Int, Return, Sizeof, While, Assign, Cond, Lor, Lan,
    Or, Xor, And, Eq, Ne, Lt, Gt, Le, Ge, Shl, Shr, Add, Sub, Mul, Div, Mod, Inc, Dec, Brak
};

/**
 * For lexical analyzer, field of identifier
 */
enum {
...
/**
 * For lexical analyzer, type of variable/function
 */
enum {
    CHAR, INT, PTR
};
...
void next() {
    ...
    while (token = *src) {
        ...
        if ('\n' == token) {
            ...
            // skip macro, because I will not support it
            while ((0 != *src) && ('\n' != *src)) {
                src++;
            }
        } else if ((('a' <= token) && ('z' >= token)) || (('A' <= token) && ('Z' >= token)) || ('_' == token)) {
            // parse identifier
            last_pos = src - 1;
            hash = token;
...
void expression(int level) {
    ...
    if (token == Num) {
        ...
        *++text = token_val;
        ...
        // sizeof is actually an unary operator now only `sizeof(int)`, `sizeof(char)` and `sizeof(*...)` are supported.
        match(Sizeof);
        match('(');
        expr_type = INT;
        ...
        *++text = (expr_type == CHAR) ? sizeof(char) : sizeof(int);
        ...
        // dereference *<addr>
        match(Mul);
        expression(Inc); // dereference has the same precedence as Inc(i++)
        ...
        *++text = XOR;
        ...
        if (*text == LC) {
            ...
            *++text = LI;
            ...
            printf("%d: bad value of pre-increment\n", line);
            exit(-1);
        }
        *++text = PUSH;
        *++text = IMM;
    ...
    while (token >= level) {
        // handle according ot current operator's precedence
        tmp = expr_type;
        if (token == Assign) {
            // var = expr;
            match(Assign);
...
void function_declaration() {
    ...
    match('{');
    function_body();
    // match('}'); consumpt the '}' in outer while
...
void global_declaration() {
    ...
    int type; // tmp, actual type for vairable
    int i; // tmp
    basetype = INT;
